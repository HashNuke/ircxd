...
| Account, away, monitor, and user-property tracking | partial | `server_authentication_test.exs`, `server_account_notify_test.exs`, `server_away_test.exs`, `server_away_notify_test.exs`, `server_monitor_test.exs`, `server_user_queries_test.exs`; application-owned account propagation, negotiated self `ACCOUNT` notification, negotiated AWAY fan-out with `305`/`306` and `away-notify`, away-aware WHOIS `301`, MONITOR add/remove/clear/list/status with `730`/`731`/`732`/`733`, connect/disconnect notifications, and `ISON`/`USERHOST` replies with `303`/`302` exist, add common-channel account changes and remaining user-property tracking |
