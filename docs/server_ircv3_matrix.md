...
| Channels (`JOIN`, `PART`, `NAMES`, `TOPIC`, `MODE`, `LIST`) | partial | `server_channels_test.exs`, `server_topic_test.exs`, `server_validation_test.exs`, `server_mode_test.exs`, `server_list_test.exs`; core state, target validation, read-only mode queries, and LIST numerics exist, add channel modes/policy |
