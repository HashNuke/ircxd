...
| Batches, multiline, history, and redaction | partial | `server_batch_test.exs`, `server_chat_history_test.exs`; client-originated `BATCH` start/end framing is relayed around tagged messages to actual recipients, bounded member-only `CHATHISTORY LATEST`/`BEFORE`/`AFTER` replies are served from in-memory channel history, while other history selectors, server-originated multiline delivery, and redaction remain |
