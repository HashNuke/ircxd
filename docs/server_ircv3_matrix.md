...
| Batches, multiline, history, and redaction | partial | `server_batch_test.exs`, `server_multiline_test.exs`, `server_chat_history_test.exs`, `server_chat_history_targets_test.exs`; client-originated `BATCH` start/end framing and `draft/multiline` messages are relayed to actual recipients, bounded member-only `CHATHISTORY LATEST`/`BEFORE`/`AFTER`/`AROUND`/`BETWEEN` replies and visible `TARGETS` discovery are served from in-memory channel history, while redaction remains |
