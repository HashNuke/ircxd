...
| Channels and identity (`JOIN`, `PART`, `KICK`, `NAMES`, `WHO`, `WHOIS`, `TOPIC`, `MODE`, `LIST`) | partial | `server_channels_test.exs`, `server_topic_test.exs`, `server_validation_test.exs`, `server_mode_test.exs`, `server_list_test.exs`, `server_multi_join_test.exs`, `server_multi_part_test.exs`, `server_names_all_test.exs`, `server_join_idempotency_test.exs`, `server_kick_test.exs`, `server_who_test.exs`, `server_whois_test.exs`; core state, idempotent and multi-target JOIN/PART, KICK removal, membership 442 validation, all-channel NAMES, WHO/WHOIS identity, target validation, read-only mode queries, and LIST numerics exist, add channel modes/policy |
