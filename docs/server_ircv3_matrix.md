...
| Channels (`JOIN`, `PART`, `NAMES`, `TOPIC`, `MODE`, `LIST`) | partial | `server_channels_test.exs`, `server_topic_test.exs`, `server_validation_test.exs`, `server_mode_test.exs`, `server_list_test.exs`, `server_multi_join_test.exs`, `server_multi_part_test.exs`; core state, multi-target JOIN/PART, target validation, read-only mode queries, and LIST numerics exist, add channel modes/policy |
