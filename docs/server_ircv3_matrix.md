...
| Capability negotiation and `CAP LS/REQ/END` | partial | `server_tagmsg_test.exs`, `server_authentication_test.exs`, `server_capability_test.exs`, `server_server_time_test.exs`; LS/ACK/NAK/END, message-tags/SASL/server-time advertisement, and negotiated per-connection capability state exist, add capability state policy cases |
