...
| Capability negotiation and `CAP LS/REQ/END` | partial | `server_tagmsg_test.exs`, `server_authentication_test.exs`, `server_capability_test.exs`, `server_server_time_test.exs`, `server_extended_join_test.exs`, `server_away_notify_test.exs`; LS/ACK/NAK/END, message-tags/SASL/server-time/extended-join/away-notify advertisement, and negotiated per-connection capability state exist, add capability state policy cases |
...
| Account, away, monitor, and user-property tracking | partial | `server_authentication_test.exs`, `server_away_test.exs`, `server_away_notify_test.exs`, `server_monitor_test.exs`, `server_user_queries_test.exs`; application-owned account propagation, negotiated AWAY fan-out with `305`/`306` and `away-notify`, away-aware WHOIS `301`, MONITOR add/remove/clear/list/status with `730`/`731`/`732`/`733`, connect/disconnect notifications, and `ISON`/`USERHOST` replies with `303`/`302` exist, add account-notify and remaining user-property tracking |
