...
| Server information (`MOTD`, `LIST`, `LUSERS`, `VERSION`, `TIME`, `ADMIN`, `INFO`, `WHOWAS`) | partial | `server_motd_test.exs`, `server_list_test.exs`, `server_lusers_test.exs`, `server_version_test.exs`, `server_time_test.exs`, `server_admin_test.exs`, `server_info_test.exs`, `server_whowas_test.exs`; configurable MOTD, channel LIST, live LUSERS, VERSION, TIME, ADMIN, configurable INFO lines through `371`/`374`, and bounded in-memory nick history through `314`/`369` exist, add remaining query commands |
