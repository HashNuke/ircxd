...
| Registration (`CAP`, `NICK`, `USER`, `PASS`, `005`, welcome numerics) | partial | `server_registration_test.exs`, `server_password_test.exs`, `server_isupport_test.exs`; handshake, ISUPPORT advertisement, duplicate nick policy, password/464 rejection, nickname validation/432, and configurable registration timeout exist; add remaining policy cases |
...
| Server information (`MOTD`, `LIST`, `LUSERS`, `VERSION`, `TIME`) | partial | `server_motd_test.exs`, `server_list_test.exs`, `server_lusers_test.exs`, `server_version_test.exs`, `server_time_test.exs`; configurable MOTD, channel LIST, live LUSERS, VERSION, and TIME numerics exist, add remaining query commands |
| Channels (`JOIN`, `PART`, `NAMES`, `TOPIC`, `MODE`, `LIST`) | partial | `server_channels_test.exs`, `server_topic_test.exs`, `server_validation_test.exs`, `server_mode_test.exs`, `server_list_test.exs`, `server_multi_join_test.exs`, `server_multi_part_test.exs`, `server_names_all_test.exs`; core state, multi-target JOIN/PART, all-channel NAMES, target validation, read-only mode queries, and LIST numerics exist, add channel modes/policy |
| Published messages (`PRIVMSG`, `NOTICE`, `TAGMSG`) | partial | `server_messaging_test.exs`, `server_isolation_test.exs`, `server_direct_message_test.exs`, `server_tagmsg_test.exs`, `server_message_tags_test.exs`, `server_message_errors_test.exs`; channel/direct routing, tag preservation, and malformed PRIVMSG/NOTICE policy exist, add target/membership validation |
