...
| IRCv3 draft support | Existing draft helper tests for metadata, chathistory, multiline, channel rename, redaction, read markers, account registration, pre-away, reactions, channel context, and extended-isupport | Partial by policy; drafts are not current completion scope |
