...
| Error numerics | partial | `test/ircxd/client_error_numeric_test.exs`, `test/ircxd/client_rfc2812_numeric_test.exs` | Audit uncommon numerics as one `Modern numeric coverage` slice. |
...
1. `Modern numeric coverage`: audit uncommon Modern IRC and RFC2812 numerics,
   then add typed events/tests in one commit.
