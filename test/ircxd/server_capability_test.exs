defmodule Ircxd.ServerCapabilityTest do
  ...
  test "returns 461 when CAP REQ has no capability list" do
    {:ok, server} = Server.start_link(port: 0)
    on_exit(fn -> stop_if_alive(server) end)
    ...
    assert :ok = Client.raw(client, "CAP", ["REQ"])
    assert_receive {:ircxd, {:irc_error, %{code: "461", target: "CAP"}}}, 2_000
  end
